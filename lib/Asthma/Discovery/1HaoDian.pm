package Asthma::Discovery::1HaoDian;
use Moose;
...
sub BUILD {
    my $self = shift;
    $self->site_id(112);
    $self->start_urls(['http://www.yihaodian.com/product/listAll-21306/', 'http://www.yihaodian.com/product/listAll-21392/', 'http://www.yihaodian.com/product/listAll-5117/', 'http://www.yihaodian.com/product/listAll-21266/']);
}
...
sub start_find_urls {
    ...
    foreach my $su ( @{$self->start_urls} ) {
        my $resp = $self->ua->get($su);
        if ( my $content = $resp->decoded_content ) {
            my $tree = HTML::TreeBuilder->new_from_content($content);
            if ( my $div = $tree->look_down(_tag => 'div', class => 'allsort sortwidth') ) {
                foreach my $dd ( $div->look_down(_tag => 'dd') ) {
                    foreach my $em ( $dd->look_down(_tag => 'em') ) {
                        if ( my $ah = $em->look_down(_tag => 'a') ) {
    ...
    if ( @$urls ) {
		...
		debug("$hdr->{Status} $hdr->{Reason} $hdr->{URL}");
                ...
                if ( my $content = $mess->decoded_content ) {
                    my $tree = HTML::TreeBuilder->new_from_content($content);
                    if ( my $ul = $tree->look_down(_tag => 'ul', class => 'itemSearchList') ) {
                        foreach my $li ( $ul->look_down(_tag => 'li', class => 'producteg') ) {
                            if ( my $item_url = $li->look_down(_tag => 'a', class => 'title')->attr('href') ) {
                                # http://www.1mall.com/item/5012590_1?ref=1_1_52_search.ctg_1
                                $item_url =~ s{\?.+}{};
				...
				if ( ! $u->in_storage ) {
                    ...
                    if ( my $ah = $tree->look_down('class', 'page_next') ) {
